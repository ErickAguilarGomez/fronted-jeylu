...
const filteredOptions = computed(() => {
  ...
  return props.options.filter(o => {
    // Search in labelKey and also email if exists
...
const handleBlur = () => {
  // Delay slightly to allow click event on option to fire
  setTimeout(() => {
    if (!props.modelValue && props.required && searchQuery.value) {
      // If required but not selected, clear input or show error
      searchQuery.value = ''
